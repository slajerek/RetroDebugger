...
#
# Wrapper over build-linux.sh. --tier commercial is the whole of it, but that
# one switch moves several things at once and this script exists to record them
# in the place someone will look. The Windows twin is
# build-windows-prod-commercial.ps1; keep the two in step.
#
...
# store build never ships symbols. The debug info is still kept, outside the
# package, under $MT_OUT/symbols.
#
# Usage: ./build-linux-prod-commercial.sh [--jobs N] [extra args passed through]
#   --jobs N   caps build parallelism (MT_BUILD_JOBS); see the note in
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --jobs) [[ "${2:-}" =~ ^[1-9][0-9]*$ ]] || { echo "ERROR: --jobs needs a positive integer" >&2; exit 2; }
                export MT_BUILD_JOBS="$2"; shift 2 ;;
        *)      ARGS+=("$1"); shift ;;
    esac
done
...
"$SCRIPT_DIR/build-linux.sh" --release --prod --tier commercial \
    ${ARGS[@]+"${ARGS[@]}"}

# No -nc suffix on a commercial deploy: the driver names it after MT_CMAKE_TARGET.
...
if [ -f "$SCRIPT_DIR/platform/Linux/prod/$(uname -m)/retrodebugger" ]; then
    echo
    echo "Package: $SCRIPT_DIR/platform/Linux/prod/$(uname -m)/retrodebugger"
