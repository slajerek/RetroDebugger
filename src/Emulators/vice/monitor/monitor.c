...
/* RD: 'G XXXX' sets PC via a maincpu trap so the GUI-driven monitor can change flow */
...
void mon_go(void)
{
    ...
    if (should_pause_on_exit_mon /* || ui_pause_active(): RD has no arch ui_pause; feature inert */) {
...
void monitor_close(int check_exit)
{
#ifdef FEATURE_CPUMEMHISTORY
    memmap_state &= ~(MEMMAP_STATE_IN_MONITOR);
#endif
    ...
    if (check_exit && exit_mon == exit_mon_quit_vice) {
		...
		LOGTODO("monitor_close: archdep_vice_exit(0)");
        archdep_vice_exit(0);
    }
...
{	
	...
	/* RetroDebugger drives the monitor through its own GUI: monitor_startup only
	   signals the debugger to enter paused mode (above) and then returns. The native
	   blocking command loop was commented out in RD's VICE 3.1 base; the VICE 3.10
	   loop is kept verbatim below (disabled) for reference. Its ui_pause_* /
	   pause_on_exit_mon machinery is part of the native-UI / vice-thread model that
	   RD (single-threaded, USE_VICE_THREAD off) does not use. */
#if 0
    if (inside_monitor) {
        /*
