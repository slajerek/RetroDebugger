...
/// c64 debugger
diskunit_context_t *c64d_get_drive_context(int driveId)
{
	return diskunit_context[driveId];
}

int c64d_get_drive_current_halftrack(int driveId)
{
	diskunit_context_t *drv = diskunit_context[driveId];
	if (drv == NULL || drv->drives[0] == NULL)
		return 0;
	
	return drv->drives[0]->current_half_track;
}

disk_image_t *c64d_get_drive_disk_image(int driveId)
{
//	if (diskunit_context == NULL)	// this is static, no need to check
//		return NULL;
...
//	LOGD("c64d_get_drive_disk_image: attach_clk=%d", drv->drives[0]->attach_clk);
//	LOGD("c64d_get_drive_disk_image: detach_clk=%d", drv->drives[0]->detach_clk);

	disk_image_t *diskImage = drv->drives[0]->image;
//	LOGD("diskImage=%x di->gcr=%x gcr=%x", diskImage, diskImage ? diskImage->gcr : 0, drv->drives[0]->gcr);
...
unsigned int c64d_get_drive_is_disk_attached(int driveId)
{
	...
	if (drv->drives[0]->GCR_image_loaded || drv->drives[0]->P64_image_loaded)
		return 1;
...
{
	diskunit_context_t *drv = diskunit_context[driveId];
	drive_set_half_track(halfTrack, 0, drv->drives[0]);
}
...
//		return NULL;
	...
	gcr_t *diskGCR = drv->drives[0]->gcr;
//	LOGD("gcr=%x", drv->drives[0]->gcr);
