...
#ifdef RETRODEBUGGER
#include "sid.h"
/* RD debugger integration: c64d_debug_mode / DEBUGGER_MODE_PAUSED come from here.
   RD runs its debugger UI on a separate thread from the emulator even with
   USE_VICE_THREAD undefined; c64d_lock/unlock_sound_mutex guards sound state
...
#ifdef RETRODEBUGGER
/* RD: debugger sets volume via 0.0..1.0 float (MASTER_VOLUME_ONE==100). */
...
#ifdef RETRODEBUGGER
        /* RD: log instead of popping a native UI error dialog. */
...
#ifdef RETRODEBUGGER
/* RD: debugger per-voice mute mask (see sid/sid.c sid_sound_machine_set_voice_mask). */
...
static int sound_run_sound(void)
{
...
#ifdef RETRODEBUGGER
    /* RD: debugger-controlled SID-run gating. When the debugger disables SID
       emulation, or disables it while in warp, skip generating SID samples but
       keep the buffer/clocks advancing (silence). Mirrors RD's live sound_run_sound
       block; SWORD->int16_t and SOUND_BUFSIZE->snddata.bufsize for 3.10. */
    ...
    if (c64d_setting_run_sid_emulation == 0
        ...
        if (cycle_based) {
            ...
            if (isWarp != 1) {
                /* in silent warp we do not need to fill the buffer */
                bufferptr = snddata.buffer + snddata.bufptr * snddata.sound_output_channels;
    ...
    if (cycle_based) {
        ...
        nr = sound_machine_calculate_samples(snddata.psid,
                                             ...
                                             &delta_t);
#ifdef RETRODEBUGGER
        /* RD volume scaling (NOTE: RD applies this in addition to the amp scaling
           below — preserved verbatim from RD's live sound.c; only affects volume<100). */
        if (volume < 100) {
        ...
        if (delta_t && !archdep_is_exiting()) {
         ...
         sound_machine_calculate_samples(snddata.psid,
                                         ...
                                         &delta_t);
#ifdef RETRODEBUGGER
        /* RD volume scaling (see note in cycle-based path above). */
        if (volume < 100) {
            for (i = 0; i < (nr * snddata.sound_output_channels); i++) {
                bufferptr[i] = (volume != 0) ? (bufferptr[i] / (100 / volume)) : 0;
            }
...
#ifdef RETRODEBUGGER
/* RD: reset the sound clock/buffer (used by the debugger on resume). */
void c64d_reset_sound_clk(void)
{
    snddata.fclk = SOUNDCLK_CONSTANT(maincpu_clk);
    snddata.wclk = maincpu_clk;
    snddata.lastclk = maincpu_clk;
...
/* RD: run sound while the emulator is paused (debugger), without main-CPU-clock
   sync — fakes a fixed delta_t (one PAL frame) so audio keeps flowing while paused.
   Ported verbatim from RD's live sound.c, adapted to 3.10: SWORD->int16_t,
   int delta_t->CLOCK, SOUND_BUFSIZE->snddata.bufsize,
   translate_text(IDGS_*)->3.10 literal strings. */
int c64d_sound_run_sound_when_paused(void)
{
    ...
    if (cycle_based) {
        ...
        if (snddata.bufptr + nr > snddata.bufsize) {
...
#ifdef RETRODEBUGGER
    /* RD: when the debugger has the machine paused, keep audio flowing via the
       paused-sound path (isPaused was provably c64d_debug_mode==DEBUGGER_MODE_PAUSED). */
...
#ifdef RETRODEBUGGER
/* RD: when set, the debugger can write to sound-chip registers without advancing
   the sound engine (used while injecting/restoring chip state). */
...
#ifdef RETRODEBUGGER
    /* RD: guard against NULL playdev (sound may not be open in the debugger). */
    if (!snddata.playdev) {
        return;
    }
#endif /* RETRODEBUGGER */
