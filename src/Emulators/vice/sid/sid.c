...
struct sound_s *fakesid_open(uint8_t *sidstate, int chipNo);	/* RD: sid_engine_t.open carries chipNo (multi-SID) */
...
void fakesid_resid_state_write(struct sound_s *psid,
                    struct sid_snapshot_state_s *sid_state);
#ifdef RETRODEBUGGER
static void fakesid_set_voice_mask(struct sound_s *psid, BYTE voiceMask);
#endif
...
uint8_t sid_read(uint16_t addr)
{
    ...
    else if (sid_stereo >= 3
        && addr >= sid4_address_start
        && addr < sid4_address_end) {
        val = sid_read_chip(addr, 3);
    }
    else if (sid_stereo >= 4
        && addr >= sid5_address_start
        && addr < sid5_address_end) {
        val = sid_read_chip(addr, 4);
    }
    else if (sid_stereo >= 5
        && addr >= sid6_address_start
        && addr < sid6_address_end) {
        val = sid_read_chip(addr, 5);
    }
    else if (sid_stereo >= 6
        && addr >= sid7_address_start
        && addr < sid7_address_end) {
        val = sid_read_chip(addr, 6);
    }
    else if (sid_stereo >= 7
        && addr >= sid8_address_start
        && addr < sid8_address_end) {
        val = sid_read_chip(addr, 7);
    }
    else {
...
#ifdef RETRODEBUGGER
/* c64 debugger: fakesid (no-SID fallback) needs a no-op voice-mask hook so the
   unconditional sid_engine.set_voice_mask() call in sid_sound_machine_init is safe
   when fakesid is the active engine. */
static void fakesid_set_voice_mask(struct sound_s *psid, BYTE voiceMask) {
}
#endif
