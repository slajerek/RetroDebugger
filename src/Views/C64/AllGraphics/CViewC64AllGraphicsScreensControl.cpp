...
void CViewC64AllGraphicsScreensControl::SetPosition(float posX, float posY, float posZ, float sizeX, float sizeY)
{
...
void CViewC64AllGraphicsScreensControl::LayoutParameterChanged(CLayoutParameter *layoutParameter)
{
...
void CViewC64AllGraphicsScreensControl::RepositionButtons()
{
	...
	// Row 1: B/W, HIRES, MULTI, charset address list
	...
	lstCharsetAddresses->SetPosition(px, py, posZ, lstFontSize * 6.5f, 65.0f / 8.0f * fontSize);
...
void CViewC64AllGraphicsScreensControl::PostRenderImGui()
{
	...
	if (isShowingContextMenu || HasContextMenuItems() || numNotHiddenLayoutParameters > 0)
	{
		...
		if (!isShowingContextMenu)
		{
			...
			onInteractiveElement = btnModeBitmapColorsBlackWhite->IsInside(mx, my)
				...
				|| lstCharsetAddresses->IsInside(mx, my);
			...
			if (!onInteractiveElement)
			{
				...
				float numLeds = btnModeMulti->IsOn() ? 3.0f : (btnModeHires->IsOn() ? 1.0f : 0.0f);
...
bool CViewC64AllGraphicsScreensControl::DoRightClick(float x, float y)
{
	...
	if (btnModeBitmapColorsBlackWhite->IsInside(x, y)
		...
		|| lstCharsetAddresses->IsInside(x, y))
	{
		return true;
	}
